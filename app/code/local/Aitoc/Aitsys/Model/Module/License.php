...
;)(nael                  } ';
    ...
    public function getService( $for = 'default' )
    {
        if (!isset($this->_service[$for]))
        {
            ...
            {
                $service = new Aitoc_Aitsys_Model_License_Service();
            }
    ...
    {
        $title = $this->_helper()->__('The license for module `%s` has been violated',$this->getModule()->getLabel());
        $description = 'Module `%s` has been uninstalled. Module license file not found or corrupted!';
        $description = $this->_helper()->__($description,$this->getModule()->getLabel());            
        try
        {
    ...
    {
        $title = $this->_helper()->__('The license for module `%s` has been violated - module will be disabled.',$this->getModule()->getLabel());
    ...
    {
        return Mage::getModel('aitsys/notification');
    }
    ...
    public function install()
    {
        ...
        if ($this->isUninstalled())
        {
            ...
            {
                $msg = $exc->getMessage();
                $msg = Mage::helper('aitsys')->__("AITOC service returned an error: %s",$msg);
                $this->addError($msg);
            }
            catch ( Exception $exc )
            {
                $msg = $exc->getCode().": ".$exc->getFile().": ".$exc->getLine().":<br/>".$exc->getMessage();
                $this->addError(Mage::helper('aitsys')->__("Unknown error. Please retry the operation again. If installation fails, contact support department.  Error code - %s",$msg));
            }
        }
        return $this;
    }
    ...
    public function upgrade()
    {
        ...
        {
            $this->addError(Mage::helper('aitsys')->__("Unknown error. Please retry the operation again. If installation fails, contact support department.  Error code - %s","$exc"));
        }
        return false;
    }
    ...
    public function uninstall( $kill = false , $check = false )
    {
        /*if ($check)
        {
            try
            {
                ...
                $service->connect();
                $service->checkDomain();
                $receivedData = $service->getValue();
                if (isset($receivedData['status']) && 'passed' == $receivedData['status'])
                {
            ...
            {
                if (!$exc instanceof Mage_Core_Model_Store_Exception)
                {
                    throw $exc;
                }
            }
    ...
    protected function _installLicenseBefore()
    {
        if (1 == Mage::app()->getRequest()->getPost('uncompatible_confirm_uninstall'))
        {
            ...
            if (!$result)
            {
                throw new Aitoc_Aitsys_Model_License_Service_Exception(Mage::helper('aitsys')->__('It was an error processing a request. Please try again.'));
            }
        }
