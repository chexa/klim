...
class Aitoc_Aitsys_Model_System_Config_Backend_Stores extends Mage_Core_Model_Config_Data
{
    protected function _beforeSave()
    {
        ...
        if (!$module)
        {
            Mage::throwException(Mage::helper('aitsys')->__(Mage::helper('aitsys')->getErrorText('seg_config_stores_module_not_found'), (string)$moduleKey));
        }
        $performer = $module->getLicense()->getPerformer();
        if (!$performer)
        {
            Mage::throwException(Mage::helper('aitsys')->__(Mage::helper('aitsys')->getErrorText('seg_config_module_license_not_found'), (string)$moduleKey));
        }
