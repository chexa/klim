...
class Mage_CatalogSearch_Model_Mysql4_Query extends Mage_Core_Model_Mysql4_Abstract
{
    ...
    public function loadByQuery(Mage_Core_Model_Abstract $object, $value)
    {
     /*   $value = str_replace("/", "", $value);
        ...
        $subQuery = $this->_getReadAdapter()->select()
            ->from($this->getMainTable(), array('query_id', 'item' => 'REPLACE (`query_text`, "/", "")'))
            ->where('store_id=?', $object->getStoreId())
            ->order('synonym_for ASC');

        $select = $this->_getReadAdapter()->select()
                    ->from(array("m"=>$this->getMainTable()), array("m.*"))
                    ->joinLeft(array("d"=>$subQuery), "d.query_id = m.query_id" )
                    ->where('synonym_for=? OR d.item=?', $value)
                    ->order('m.query_id DESC')
                    ->limit(1);
