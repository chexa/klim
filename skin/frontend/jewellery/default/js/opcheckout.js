...
Checkout.prototype = {
    ...
    setLoadWaiting: function(step, keepDisabled) {
        if (step) {
            ...
            $$('body')[0].insert($(step+'-please-wait'));

            var wWidth  = $(step+'-please-wait').getWidth();
            var wHeight = $(step+'-please-wait').getHeight();

            $(step+'-please-wait').setStyle({left:'50%', top:'50%', position:'fixed',"margin-left": - wWidth / 2 + "px", "margin-top": - wHeight / 2+ "px"});
            Element.show(step+'-please-wait');

			var overlayHeight;
			overlayHeight = $('wrap').getHeight();
			 $('wait-overlay').show().setStyle({opacity:.6, height:overlayHeight+'px'
			 });
...
Review.prototype = {
    ...
    save: function(){
        ...
        var params = 'payment[method]=checkmo';//Form.serialize(payment.form);
