...
implements Aitoc_Aitsys_Abstract_Model_Interface
{
    ...
    public function saveData($aData, $aModuleHash = array(), $clearCache = true , $hideEvents = false)
    {
        ...
        if ($aModuleHash)
        {
            foreach ($aModuleHash as $sFile => $sFullPath)
            {
                ...
                if ($this->_checkFileSaveWithContent($sFullPath, $sFileContent))
                {
    ...
    protected function _saveAllFileContent()
    {
        ...
        foreach ($this->_aFileWriteContent as $sFilePath => $sFileContent)
        {
            ...
            fclose($fh);
            $this->tool()->filesystem()->grantAll($sFilePath);
        }
    ...
    protected function _checkFileSaveWithBackup($sFileName, $sExtenstion, $aReplaceData)
    {
        ...
        /*
        NOW WE DO NOT NEED TO CHANGE INHERITANCE
        if ($aReplaceData) // must change inheritance
        {
            if ('xml' == $sExtenstion)
            {
                foreach ($aReplaceData['search'] as $index => $search)
                {
                    $sFileContent = preg_replace('/\>'.$search.'\</im','>'.$aReplaceData['replace'][$index].'<',$sFileContent);
                }
            }
            else
            {
    ...
    protected function _addError($sFilePath, $sType)
    {
        ...
        $this->_aErrorList[] = Mage::helper('aitsys')->__($sMessage, $sFilePath);
    ...
    public function getAllowInstallErrors()
    {
        ...
        if ($aAitocModuleList)
        {
            ...
            if ($sHasRewritePathOld) // has old version
            {
                ...
                {
                    $sErrorMsg = Mage::helper('aitsys')->__('Module can not be installed because you have outdated version of Checkout Fields Manager installed. Please contact AITOC for updated version of Checkout Fields Manager to resolve this issue.');
