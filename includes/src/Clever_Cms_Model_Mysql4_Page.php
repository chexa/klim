...
class Clever_Cms_Model_Mysql4_Page extends Mage_Cms_Model_Mysql4_Page
{
    ...
    public function changeParent(Mage_Cms_Model_Page $page, Mage_Cms_Model_Page $newParent, $afterPageId = null)
    {
        ...
        if ($newParent->getIdentifier() == Mage::getStoreConfig('cms/clever/root_cms_page', Mage::app()->getStore()->getId())) {
            $newParentIdentifier = '';
        } else {
            $newParentIdentifier = $newParent->getIdentifier();
        }

        $newIdentifier = trim($newParentIdentifier.'/'.array_pop($identifiers), '/');
